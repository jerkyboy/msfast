...
{
    public class WebResourceBufferValidator : DataValidator<ValidationResults<SourceValidationOccurance>>
	{
		Regex webresource = new Regex("<\\s*(?i:\\s*s\\s*c\\s*r\\s*i\\s*p\\s*t)[^>]*(WebResource|ScriptResource).axd[^>]*>.*?<\\s*/?\\s*(?i:\\s*s\\s*c\\s*r\\s*i\\s*p\\s*t)[^>]*>", RegexOptions.IgnoreCase | RegexOptions.Compiled);
        ...
        public override ValidationResults<SourceValidationOccurance> ValidateData(ProcessedDataPackage package)
        {
            ...
            MatchCollection mc = webresource.Matches(data.PageSource);
