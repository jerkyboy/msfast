...
{
    public class LinkJSAtBottomValidator : DataValidator<ValidationResults<SourceValidationOccurance>>
    {
        #region Validator Members

        Regex linkinsidesideofhead = new Regex("<\\s*(?i:\\s*s\\s*c\\s*r\\s*i\\s*p\\s*t)[^>\\)\\(]*(?i:\\s*s\\s*r\\s*c\\s*=)[^>]*>", RegexOptions.IgnoreCase | RegexOptions.Compiled | RegexOptions.Singleline);
