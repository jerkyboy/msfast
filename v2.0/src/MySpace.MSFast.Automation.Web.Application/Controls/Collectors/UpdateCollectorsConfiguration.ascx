<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UpdateCollectorsConfiguration.ascx.cs" Inherits="MySpace.MSFast.Automation.Web.Application.Controls.Collectors.UpdateCollectorsConfiguration" %>
...
table.updateExecConf td{padding:4px 0 0 0;}
table.updateExecConf td.td1{width:20px;padding:0 0 0 6px}
table.updateExecConf td.td2{width:240px;}
table.updateExecConf td.td3{width:240px;}
.addvar{position:relative;padding:10px 0 10px 24px;width:120px;display:block}
</eyf:StyleLiteral>
...
function saveAttr(tr){
    ...
    if(tr.attr("latestval") != v){
        var f = tr.parents("div.cnfg").find(".afUpdateCollectorsConfiguration");
...
function removeAttr(tr){
    var k = tr.find("input[name=k]").val();
    var f = tr.parents("div.cnfg").find(".afRemoveCollectorsConfiguration");
    f.find("input[name=k]").val(k);
    f.submit();
}
function addnewarg(t){
    var b = t.parent().find(".updateExecConf")
    b.find("tr:last").before(bindrows(b.find("tr.template").clone().removeClass("template").attr("lastval","").attr("originalval","")));
}
function bindrows(t){
...
<div class="cnfg">
<eyf:Title runat="server" Text="Variables" icon="variable"></eyf:Title>
...
<asp:Repeater ID="rptConfiguration" runat="server">
    ...
    <ItemTemplate>
        <asp:Literal ID="ltTR" runat="server" />
            <td class="td1" width="50px"><a href="#" onclick="var t=$(this).parents('tr:first');removeAttr(t);t.remove();return false;"><span class="ico ico-delete-variable"></span></a></td>
    ...
    <FooterTemplate>
        <tr class="template newval">
            <td class="td1"><span><a href="#" onclick="var t=$(this).parents('tr:first');removeAttr(t);t.remove();return false;"><span class="ico ico-delete-variable"></span></a></span></td>
            <td class="td2"><eyf:InputText ID="itKey" runat="server" name="k" /></td>
            <td class="td3"><eyf:InputText ID="itVal" runat="server" name="v" /></td>
            <td class="td4"></td>
        </tr>
    ...
    <a href="#" onclick="addnewarg($(this));return false;" class="addvar"><span class="ico ico-add-variable"></span> Add Variable</a>
    </FooterTemplate>
</asp:Repeater>
 </div>