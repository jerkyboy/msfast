...
{
    public class JSLibrariesValidation : DataValidator<ValidationResults<SourceValidationOccurance>>
	{
		...
		public override void Init(Dictionary<string, string> config)
		{
			...
			regex = new Regex("<\\s*(?i:\\s*s\\s*c\\s*r\\s*i\\s*p\\s*t)[^>]*/" + libraries + "(.*?)[^>]*>.*?<\\s*/?\\s*(?i:\\s*s\\s*c\\s*r\\s*i\\s*p\\s*t)[^>]*>", RegexOptions.IgnoreCase | RegexOptions.Compiled);
		}
