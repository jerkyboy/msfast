...
namespace MySpace.MSFast.DataProcessors.CustomDataValidators.PageSourceValidators.WebFrameworksMatrix
{
    public class OldWordBreakersValidation : DataValidator<ValidationResults<SourceValidationOccurance>>
	{
		private Regex regex = null;

		public override void Init(Dictionary<string, string> config)
		{
			String scripts = config["scripts"];
			regex = new Regex("<\\s*(?i:\\s*s\\s*c\\s*r\\s*i\\s*p\\s*t)[^>]*>.*?" + scripts + "+.*?<\\s*/?\\s*(?i:\\s*s\\s*c\\s*r\\s*i\\s*p\\s*t)[^>]*>", RegexOptions.IgnoreCase | RegexOptions.Compiled);
		}
